...
def ask_qwen(system_prompt, user_prompt):
    try:
        ...
        return json.dumps({"matches": [], "confidence": "LOW"})
...
def validate_result(result, available_items):
    ...
    if not isinstance(matches, list):
        return False
    if any(not isinstance(item_id, str) for item_id in matches):
        return False
    if confidence not in {"LOW", "MEDIUM", "HIGH"}:
        return False

    valid_ids = {item.get("id") for item in available_items if isinstance(item, dict) and "id" in item}
...
def display_matches(result, available_items):
    ...
    for item_id in matches:
        item = next((entry for entry in available_items if entry.get("id") == item_id), None)
        if item is None:
            continue
        print()
...
def main():
    ...
    save_result(result, Path("output") / "match_result.json")
